...
# from dlp import DataReader
# from deep_lear.general_utils.data_read import DataReader    
from deep_learning_package.general_utils.data_read import DataReader

class Mine_GradientDescent_LinaerRgression:
    """
    BACTCH GRADIENT DESCENT OPTIMIZATION ALGORITHM FOR LINEAR REGRESSION.:

    This is a simple implementation of linear regression using gradient descent optimization algorithm.
    The class takes in the learning rate, number of epochs, initial values for the intercept (b) and the coefficient (w) as input parameters.
    The fit method is used to train the model on the training data, and the predict method is used to make predictions on the test data.
    The fit method calculates the predicted values (y_hat) using the current values of b and w, computes the error between the predicted and actual values,
    and updates the values of b and w using the gradients of the cost function with respect to b and w.
    The cost function used here is the mean squared error (MSE). The history of the cost function, b, and w is stored in the J_w_b_history dictionary for each epoch.
    """
